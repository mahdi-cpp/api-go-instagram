...
import (
	...
	"github.com/mahdi-cpp/api-go-instagram/utils"
...
var username = "Go-Instagram"

// var directory = "/home/mahdi/.local/bin/" + username
var directory = "/var/Go-Instagram/" + username
	...
	for _, file := range files {
		if strings.Contains(file, ".txt") {
			...
			for _, photo := range photos {
				...
				width, height := utils.GetImageDimension(directory + "/" + photo + ".jpg")
				fmt.Println("Width:", width, "Height:", height)

				// Resize the cropped image to width = 200px preserving the aspect ratio.
				src, err := imaging.Open(directory + "/" + photo + ".jpg")
				if err != nil {
					log.Fatalf("failed to open image: %v", err)
				}
				src = imaging.CropAnchor(src, 200, 200, imaging.Center)
				// Save the resulting image as JPEG.
				err = imaging.Save(src, "/tmp/mahdi/"+photo+"_thumbnail.jpg")
				if err != nil {
				...
				media.Thumbnail = photo + "_thumbnail.jpg.jpg"
				...
				//config.DB.Create(&model.Tag{Name: "@mahdiabdolmaleki", MediaRefer: media.ID, X: 600, Y: 250})
			}
		}
	}
}
